...
data = read_csv('../checkpoint_folder/DCTest_linear_rfadj/extracted_data.csv')
data['ones'] = 1
...
gain_no_vdd = (out - constant_effects - cm_effects) / d[in_col_names[0]] - diff_vdd_effect
gain_no_rfadj = (out - constant_effects - cm_effects) / d[in_col_names[0]] - diff_rfadj_effect



plt.plot(d[rf_decimal_col_name], gain_no_vdd, 'x')
plt.title('Estimated gain vs rfadj with vdd effect removed')
...
plt.plot(d[vdd_col_name], gain_no_rfadj, 'x')
plt.title('Estimated gain vs vdd with rfadj effect removed')
plt.xlabel('vdd')
plt.ylabel('estimated gain')
plt.ylim((0, 3000))
plt.grid()
